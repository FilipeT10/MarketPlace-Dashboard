...
class EditarPedido extends React.Component {
  ...
  componentDidMount() {
    ...
    if(this.product.endereco.referencia == "Loja Física" && this.product.endereco.cep == "Loja Física" && this.product.endereco.numero == 0){
      this.setState({isCheckedEntregarLoja: true})
    }else{
      this.setState({values: {...this.state.values, ...this.product.endereco}})
    }
  }
  ...
  savePedidos = () => {
    ...
    if(possuiError == false){
      ...
      console.log(json)
      ServicePedidos.editPedidos(this.product._id, json).then(response => {
...
export default () => {
  ...
  return (
      <EditarPedido product={location.state.values} />
  )
}
