...
class EditarPedido extends React.Component {
  ...
  state = {
    ...
    subcategorias: [],
    produtos: [],
    produtosPedido: this.product.produtos,
    ingredientesSelected: [],
    values: { ...this.product, tipopagamentos: this.product.tipoPagamento },
    frete: 5.99,
    selectedProduto: {},
    quantidadeProduto: 1,
    modalVisible: false,
  ...
  componentDidMount() {
    ...
    if (
      this.product.endereco.referencia == 'Loja Física' &&
      this.product.endereco.cep == 'Loja Física' &&
      this.product.endereco.numero == 0
    ) {
      this.setState({ isCheckedEntregarLoja: true });
    } else {
      this.setState({
        values: { ...this.state.values, ...this.product.endereco }
      });
    }
  }
  ...
  adicionarProduto = (product) => {
    ...
    this.setState({
      values: { ...this.state.values, cor: undefined, tamanho: undefined }
    });
  ...
  savePedidos = () => {
    ...
    if (possuiError == false) {
      ...
      console.log(json);
      ServicePedidos.editPedidos(this.product._id, json)
        .then((response) => {
  ...
  editProduct = (produto) => {
    ...
    this.setState({
      values: { ...this.state.values, ...this.state.produtosPedido[produto] }
    });
    this.setState({
      editProduct: produto,
      detalhesProduto: false,
            ...
            <Box sx={{ pt: 3 }}>
              <Box sx={{}}>
                <div>
                  ...
                  <Card>
                    <CardHeader subheader="Pedido" title="Editar" />
                    <Divider />
                    {loading ? (
                      <LinearProgress />
                    ) : (
...
export default () => {
  ...
  return <EditarPedido product={location.state.values} />;
};
